...
- 분산 시스템에서 애플리케이션의 고가용성을 위해 **리플리케이션**가 중요하지만 구현 시 해결해야 할 문제 존재
  - 리플리케이션의 동작 구현 어려움
  - 애플리케이션의 성능 저하 유발
...
- **리더**와 **팔로워**는 **ISR(InSyncReplica)** 논리 그룹에 속함
  - 해당 그룹 안에 속한 팔로워들만 새로운 리더가 될 자격을 가짐
  - ISR 그룹에 속하지 못한 팔로워는 리더가 될 자격을 가질 수 없음
  - ISR 그룹에 속한 팔로워는 새로운 리더가 될 수 있도록 지속적으로 리더와의 데이터 일치
...
- 커밋되었다는 것은 리플리케이션 팩터 수의 리플리케이션이 전부 메세지를 저장했다는 의미
- 데이터 일관성을 위해 커밋된 메세지만 컨슈머는 소비
  - 커밋되지 않은 메세지를 컨슈머가 읽을 경우, 리플리케이션이 동작하기 전에 리더가 죽고, 새로운 리더가 선출되었을 때, 리더가 죽기 전 메세지를 읽어간 컨슈머와 리더가 죽은 후 메세지를 읽어간 컨슈머 간 데이터 정합성 문제 발생
- **replication-offset-checkpoint** 파일
  ...
  - 특정 토픽이나 파티션에 복제가 되지 않는 등의 문제가 발생하면, 모든 브로커의 체크포인트 파일을 확인하여 판단

- 리더와 팔로워의 리플리케이션 동작
  - 모든 읽기와 쓰기를 담당하는 리더가 리플리케이션을 위해 팔로워와 많은 통신을 주고 받으면 성능 문제 발생
  - 카프카는 리더와 팔로워 간 통신을 최소화하여 리더의 부하를 줄임
  - 단계
    1. 프로듀서가 리더에게 새로운 메세지 전송
    2. 팔로워는 리더에게 오프셋 메세지 fetch 요청을 보낸 후 새로운 메세지 전송 인지 (리더는 fetch 요청을 통해 팔로워가 요청을 보냈다는 사실 인지)
    3. 리더는 팔로워에게 새로운 메세지를 보냈다는 사실만 알고, 실제 리플리케이션이 성공했는지는 모름 (RabbitMQ는 ack 메세지를 통해 성공 여부 알림)
    4. 또다른 새로운 메세지가 들어오고, 팔로워들로부터 새로운 메세지를 요청 받음
    5. 리더는 이 요청을 토대로, 기존 메세지 리플리케이션이 성공 여부 판단  
       5-1. 팔로워 요청 오프셋이 새로 들어온 메세지 오프셋과 같으면 이전 메세지 오프셋에 커밋 표시  
       5-2. **하이워터마크**를 증가 시킴  
       5-3. 리플리케이션 요청에 대한 응답으로 이전 메세지가 커밋되었다고 전달
       5-4. 팔로워는 리더의 응답 메세지로 이전 메세지가 커밋되었음을 표시  
       (팔로워 요청 오프셋이 새로 들어온 메세지 오프셋보다 이전이면 해당 팔로워의 리플리케이션이 실패했다고 인지)
...
- 팔로워가 하이워터마크 증가 메세지를 받지 못한 상태에서 팔로워가 죽는 경우 (리더에포크 없음)
  ...
  - 메세지 2를 받아 오프셋 2가 되고, 팔로워로부터 요청을 받아 하이워터마크 증가
    ...
    | 리더 | 2 | 2 | message2 |
    | 팔로워 | 2 | 1 | message2 |
  - 리더가 하이워터마크를 증가시키는 응답을 받기 전에 팔로워 죽음
    - 팔로워는 자신이 가지고 있는 메세지 중 자신의 워터마크보다 높은 메세지들은 실뢰할 수 없다고 판단하고 지움
    - 리더에게 오프셋 1번 새로운 메세지를 fetch 하는 요청 보냄
...
- Fetch Request
  ...
  - 팔로워는 Fetch Request를 fetch 받아야할 offset을 포함하여 발행
...
- Committing Partition Offsets
  ...
  - ISR 그룹에 속한 모든 팔로워들이 특정 offset 레코드를 전부 fetch할 경우, 해당 offset 레코드는 commit되어 컨슈머가 소비할 수 있는 상태가 될 수 있음
  - commit되어 컨슈머가 소비할 수 있는 상태는 **하이워터마크**로 표현
  - 리더는 팔로워가 보낸 Fetch Request에 포함된 offset을 통해 해당 팔로워가 어디까지 리플리케이션 되었는지 판단  
...
- Advancing the Follower High Watermark
  ...
  - 리더는 Fetch Response에 **하이워터마크**를 함께 보냄으로써 현재 상태를 팔로워에게 알림
  - 리더의 **하이워터마크**는 Fetch Response에 의해서만 팔로워에게 전달되기 때문에 실제 리더의 **하이워터마크**는 팔로워가 알고 있는 **하이워터마크**보다 항상 클 가능성이 있음 (팔로워는 항상 Fetch Response를 받을 당시의 값만 알기 때문)
...
- Handling Leader Failure
  ...
  - 리더 선출과 선출된 리더를 팔로워들에게 알리는 역할은 control plane에서 담당
  - data plane에서는 기존 리더 장애와 새로운 리더 선출 과정에서 발생하는 데이터 유실이 가장 큰 문제
  - 이를 위해 리더는 ISR 그룹에서 선출
    → ISR 내 팔로워는 가장 마지막에 커밋된 offset 데이터 보장
...
- Temporary Decreased High Watermark
  ...
  - **하이워터마크**는 Fetch Request으로 비동기적으로 업데이트 되기 때문에 팔로워마다 **하이워터마크**가 다를 수 있음
  - 특정 팔로워는 기존 리더로부터 최신 **하이워터마크**를 전달 받았으나 새로운 리더는 해당 **하이워터마크**를 가지고 있지 않을 가능성이 있음
  - ISR 그룹은 가장 마지막으로 커밋된 오프셋 데이터를 모든 팔로워가 들고 있음을 보장하지만 최신 **하이워터마크**가 모든 팔로워에게 전달되는 것을 보장하지는 않음 (**하이워터마크**는 eventual consistency)
  - 최신 **하이워터마크** 값을 가진 팔로워의 요청에 새로운 리더는 응답할 수 없기 때문에 `새로운 리더 하이워터마크 ~ 최신 하이워터마크`의 요청에 대해서는 OFFSET_NOT_AVAILABLE 에러 발생
  - 새로운 리더가 **하이워터마크**를 새로운 값으로 갱신할 때까지(가장 마지막으로 커밋된 오프셋) 팔로워는 계속 fetch 시도
...
- Partition Replica Reconciliation
  ...
  - 리더로 선출된 직후, 특정 팔로워는 새로운 리더에게 없는 커밋되지 않은 레코드를 가지고 있을 수 있음 (팔로워 마다 Fetch Request를 보낸 정도가 다름)
    → 새로운 리더의 **하이워터마크**가 다른 팔로워보다 작은 이유
  - **하이워터마크** 불일치 문제가 발생할 경우, 새로운 리더는 기존 리더에게서 리플리케이션한 offset을 요청을 통해 받아야만 **하이워터마크**를 갱신 가능
...
- Fetch Response Informs Follower of Divergence
  ...
  - 현재 리더(epoch)의 offset과 일치하지 않는 요청을 받았을 경우, 새로운 리더의 로그 데이터와 비교하여, 요청한 offset이 유효하지 않은지 확인
...
- Handling Failed or Slow Followers
  ...
  - 팔로워의 리플리케이션의 속도가 느릴 경우, 설정된 임계 값 시간 이상 리플리케이션 요청이 오지 않을 때 ISR 그룹에서 제거함으로써 리더가 **하이워터마크**를 계속 증가시키며, 컨슈머가 메세지를 소비할 수 있도록 함
